#!/usr/bin/env python3
"""Bakes scripts/.final-review-apply.json (the 72 ACCEPT decisions from
final-review-audit.py) into lib/words.ts's `exercisePrompt`/`exercisePromptZh`
fields. Only ever touches those two fields on an already-ACCEPTed id.

Backs up lib/words.ts to lib/words.ts.bak-<UTC timestamp> before writing,
then prints an id-by-id old -> new report.

Usage:
    python3 scripts/apply-final-review.py
"""
import datetime
import json
import os
...
APPLY_PATH = os.path.join(SCRIPT_DIR, '.final-review-apply.json')
...
def main():
    if not os.path.exists(APPLY_PATH):
        sys.exit(f'{APPLY_PATH} not found.')
